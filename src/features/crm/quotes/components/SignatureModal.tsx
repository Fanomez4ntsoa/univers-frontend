...
import { useSignQuote } from '../hooks/useQuotes'
...
export default function SignatureModal({ open, onClose, quoteId }: SignatureModalProps) {
  ...
  const handleSubmit = () => {
    ...
    signMutation.mutate(
      { id: quoteId, signature_image: signatureImage, signed_by: signedBy, signed_at: new Date().toISOString().split('T')[0] },
      {
        onSuccess: () => { toast.success('Devis signé'); onClose() },
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Nom du signataire *</label>
          ...
          <div>
            ...
            <Button type="button" variant="ghost" size="sm" onClick={clearCanvas} className="mt-1 text-xs">
              Effacer
            </Button>
          </div>
