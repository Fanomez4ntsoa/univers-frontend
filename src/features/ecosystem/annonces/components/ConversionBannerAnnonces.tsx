// ConversionBannerAnnonces.tsx — Fidèle à Emergent UniversalBanner variant="particuliers"

import { Link } from 'react-router-dom'
import { motion } from 'framer-motion'

export default function ConversionBannerAnnonces() {
  return (
    <motion.div
    ...
    >
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px' }}>
        <div
          ...
          style={{ backgroundColor: '#EEF2FF', borderLeft: '4px solid #E8650A' }}
        >
          <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 pl-2">
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-lg lg:text-xl leading-tight mb-1" style={{ color: '#0F172A' }}>
                Dépose ton annonce gratuitement
              </h3>
              <p className="text-sm lg:text-base leading-relaxed mb-1" style={{ color: '#374151' }}>
                Vends ou achète facilement entre particuliers
              </p>
              <p className="text-xs text-gray-500 mt-1">
                ✓ Gratuit · ✓ Sans engagement · ✓ Annonce en 2 minutes
              </p>
            </div>
            <Link to="/listings" className="flex-shrink-0 w-full sm:w-auto">
              <motion.button
